...
var _ = Describe("External tests", func() {
	...
	steps.Define(func(define Definitions) {
		...
		define.Then(`^the command says that the rewritten image will be pushed$`, func() {
			...
			Eventually(CommandSession.Out).Should(Say(fmt.Sprintf("harbor-repo.vmware.com/tanzu_isv_engineering/tiny:tiniest => harbor-repo.vmware.com/tanzu_isv_engineering_private/tiny:%s \\(sha256:[a-z0-9]*\\)", customTag)))
		})
